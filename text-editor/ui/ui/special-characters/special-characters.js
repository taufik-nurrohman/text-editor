/*!
 * ===========================================================
 *  SPECIAL CHARACTERS PLUGIN FOR USER INTERFACE MODULE 1.0.0
 * ===========================================================
        ...
        ui.tool(symbol, {
            ...
            click: function(e, $) {
                ...
                return ui.drop(symbol + ' ' + symbol + uniq, function(drop) {
                    ...
                    r = _dom.children;
                    _el(style, h + '{max-height:20em;overflow:auto}' + h + ' div{max-width:20em;margin:0 0 ' + q[2] + 'px;background:rgba(0,0,0,.025);border-color:inherit;font-size:100%;overflow:hidden}' + h + ' div[title]:before{content:attr(title);display:block;padding:.5em;background:' + q[0] + ';border-bottom:1px solid;border-color:inherit}' + h + ' a{float:left;width:10%;height:2em;line-height:2em;text-align:center;color:inherit;text-decoration:none;overflow:hidden;white-space:nowrap;font-family:serif}' + h + ' a img{display:block;width:60%;height:60%;max-width:none;max-height:none;min-width:0;min-height:0;margin:20%;background:0 0;border:0;outline:0;box-shadow:none;border-radius:0}' + h + ':hover a:focus{background:0 0;color:inherit}' + h + ' a:focus,' + h + ' a:hover,' + h + ':hover a:hover{outline:0;background:' + q[1] + ';color:' + q[0] + '}');
                    for (i in p) {
